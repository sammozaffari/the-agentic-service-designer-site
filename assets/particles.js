// Antigravity particle field — vendored from ~/Documents/github/testing/antigravity-website
// (demo_standalone.html export; THREE.js r128 loaded on demand from cdnjs).
// Site patches: ink-tone palette on paper, background gradient disabled, guarded init.
(function() {
  // Site integration: run only where a hero mounts the container; honor motion + size preferences.
  if (!document.querySelector('.particles-container')) return;
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  if (window.matchMedia('(max-width: 720px)').matches) return;
  // 1D Hermite Noise Generator
  class Noise1D {
  constructor() {
...
void main() {
  ...
  // Define 4 animated floating color centers (distorted spots)
...
uniform int uShapeType; // 0 = Capsule, 1 = Character/Text
...
`;
  ...
  class AntigravityParticles {
    constructor(container) {
      ...
      // Configuration values serialized from designer
      this.ringRadius = 0.175;
      this.ringWidth = 0.011;
      this.ringWidth2 = 0.107;
      this.ringDisplacement = 0.53;
      
      this.color1 = new THREE.Color("#171817");
      this.color2 = new THREE.Color("#6e6e69");
      this.color3 = new THREE.Color("#bbbbb6");
      this.colorScheme = 1;
      
      this.mouseSpeedActive = 8;
      this.mouseSpeedIdle = 4;
      this.noiseSpeed = 0.5;
      this.noiseDispersion = 0.03;
      this.density = 230;
      this.uAlpha = 1;
      this.particleScale = 0.59;
      
      this.shapeType = 0;
      this.shapeName = "capsule";
      this.svgDataUrl = "";
      this.svgFileName = "";
      this.rotateParticles = 1;
      this.customText = "ANTIGRAVITY";
      this.charAtlasTexture = null;
      
      this.showBg = false;
      this.bgSpeed = 0.5;
      this.bgRibbon = 0.4;
      this.bgColor1 = new THREE.Color("#f472b6");
      this.bgColor2 = new THREE.Color("#ec4899");
      this.bgColor3 = new THREE.Color("#38bdf8");
      this.bgColor4 = new THREE.Color("#818cf8");
    ...
    init() {
      ...
      const minDistance = (this.density) * (2 - 10) / 300 + 10;
      const maxDistance = (this.density) * (3 - 11) / 300 + 11;
      ...
      if (this.shapeType === 2) {
        this.updateShapeAtlas(this.shapeName, this.svgDataUrl);
      } else {
        this.updateCharacterAtlas(this.customText);
      }
      
      window.addEventListener("mousemove", e => this.onMouseMove(e));
      window.addEventListener("mouseleave", () => { this.mouseIsOver = false; });
    ...
    updateCharacterAtlas(charsString) {
      ...
      if (this.charAtlasTexture) this.charAtlasTexture.dispose();
    ...
    updateShapeAtlas(shapeName, svgDataUrl = null) {
      const canvas = document.createElement("canvas");
      canvas.width = 512;
      canvas.height = 512;
      const ctx = canvas.getContext("2d");
      ...
      if (shapeName === "capsule") {
        ...
        drawHexagon();
        uploadTexture();
      } else if (shapeName === "svg" && svgDataUrl) {
        const img = new Image();
        img.onload = () => {
          const maxDim = 380;
          let dw = img.naturalWidth || img.width || 400;
          let dh = img.naturalHeight || img.height || 400;
          if (dw > dh) {
            dh = (dh / dw) * maxDim;
            dw = maxDim;
          } else {
    ...
    onResize() {
      ...
      this.renderMaterial.uniforms.uPixelRatio.value = window.devicePixelRatio;
